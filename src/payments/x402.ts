...
export const PAID_ROUTES: Array<{ path: string; price: number; description: string; mode?: import("../pipeline/index.js").ForceMode }> = [
  ...
  { path: "/v1/asset", price: 0.1, description: "Studio asset: a 1200x675 branded hero image painted from your words, with your title and subtitle set in real type — for listings, launch posts and headers.\nProvide: 1. the scene in plain words; 2. optional title; 3. optional subtitle." },
  { path: "/v1/brandkit", price: 0.1, description: "Studio brand kit: ready-to-use design tokens — a four-colour palette, a display+mono type pairing, one drawable mark concept and motion timing. From an OKX.AI agent the palette is read from its own avatar.\nProvide: an agent id (e.g. 2013), its okx.ai/agents/… link, or the brand described in a sentence." },
  { path: "/v1/restyle", price: 0.1, description: "Studio restyle: deterministic image edits — reframe to any size with attention-aware cropping, enlarge up to 2x by Lanczos resampling, and flatten transparency onto a flat colour. Real pixel transforms only, no generative fill.\nProvide: 1. an https image_url; 2. width+height to reframe, or scale up to 2, or a background hex." },
  { path: "/v1/forge", price: 0.1, description: "Studio forge: a mint-ready package for your artwork — ERC-721 metadata JSON, CIDv1 content ids and sha256 hashes for image and metadata, hosted copies of both files. Packaging only; it does not pin or mint.\nProvide: 1. an https image_url; 2. name; 3. description; optional attributes and external_url." },
